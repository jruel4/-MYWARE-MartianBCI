...
class GradDescSarsaAgent:
    ...
    def __init__(self,_EPSILON=1.0,_ALPHA=0.05,_GAMMA=0,_LAMBDA=0.6,_ALPHA_DECAY=1.0):
    ...
    def build_main_graph(self):            
        ...
        expected_reward_tmp = tf.reduce_sum(tf.multiply(tf.transpose(weights), tf.transpose(z_trace)))
        ...
        with tf.control_dependencies([weights_tmp]):
            with tf.name_scope("Z_Upd"):
                z_trace_tmp = tf.transpose(bin_features) + (z_trace * self.mLAMBDA + self.mGAMMA) #decay, then update
        ...
        #UPDATES
        with tf.control_dependencies([err_delta_1, weights_tmp]):
