...
G_logdir = G_logs + 'ValueFunction_MaybeWorking5_RandomKernelInit_4\\'
...
gamma_decay = tf.constant(1.0, name='GammaDecay')

#lbl=(current_reward + gamma_decay * next_predicted_value)
lbl=tf.placeholder(dtype=tf.float32,shape=[None,1])
loss = tf.losses.mean_squared_error(
    labels=lbl,
    predictions=next_predicted_value
    )
...
'''
# Greedy select - Calculate both A0 and best Q(S0,A0)
s=time.time()
...
arg_val = sess.run(next_predicted_value, feed_dict_greedy_select)
...
if False:
    ...
    feed_dict_tst = {
        specgram:spect,
        audio:np.asarray([[1,1,1,1]])
        }
    # Log
    d0,do0,o0 = sess.run([dense0,dropout0,output_layer], feed_dict=feed_dict_tst)
    d0_act.append(d0)
    do0_act.append(do0)
    o0_act.append(o0)
    b_w = dict()
    ...
    feed_dict_tst1 = {
        specgram:spect,
        audio:np.asarray([[1,1,1,1]]),
        lbl:np.asarray([[100601.0]])
        }
    
    d0,do0,o0,l0 = sess.run([dense0,dropout0,output_layer,loss], feed_dict=feed_dict_tst1)
    d0_act.append(d0)
    do0_act.append(do0)
    o0_act.append(o0)
    l0_act.append(l0)
    ...
    d0,do0,o0,loss1 = sess.run([dense0,dropout0,output_layer,loss], feed_dict=feed_dict_tst1)
    d0_act.append(d0)
    do0_act.append(do0)
    global o0_act
    o0_act = list(o0)
...
'''
    continue
    #
    #plt.pcolormesh(np.reshape(i,[5,50]))
