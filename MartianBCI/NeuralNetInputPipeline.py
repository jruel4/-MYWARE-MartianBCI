...
def input_fn_live(local_queue,test=False):
    ...
    for i in range(local_queue.qsize()):
        if test:
...
#        q_len.append(local_queue.qsize())
    ...
    return {'specgram':tf.constant(np.asarray(eeg_feat)),
            'audio': tf.constant(0, name='audio_input')}, {'next_state':tf.constant(np.asarray(label))}
...
#    queues = [EEGInputQueue, AudioInputQueue]
    ...
    mnist_classifier = learn.Estimator(
            model_fn=cnn_model_EEG_0,
            model_dir=".\\Logs\\NNTesting11_NN_EEG",
